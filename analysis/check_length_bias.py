...
def check_length_bias():
    file_path = "data/TEXT/processed/novelty_scores.csv"
    ...
    print("\n--- Correlation with Novelty (PMI / Normalized) ---")
    # user_surprise is now the Normalized version (cond - base)
    ...
    # Check raw if available
    if 'user_surprise_raw' in df.columns:
        print("\n--- Correlation with Raw Surprisal ---")
        u_corr_raw = df['user_len'].corr(df['user_surprise_raw'])
        a_corr_raw = df['ai_len'].corr(df['ai_surprise_raw'])
        print(f"User Length vs Raw Corr: {u_corr_raw:.3f}")
        print(f"AI Length vs Raw Corr:   {a_corr_raw:.3f}")
